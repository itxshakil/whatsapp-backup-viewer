...
export const AnalyticsView: React.FC<AnalyticsViewProps> = React.memo(({ messages, participants }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-wa-bg p-4 md:p-8 pb-12 md:pb-20">
      <div className="max-w-4xl mx-auto space-y-6">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-wa-text-primary mb-6 flex items-center gap-2">
        ...
        <div className="bg-wa-panel-bg p-6 rounded-xl shadow-sm border border-wa-divider">
          <h3 className="text-sm font-semibold text-wa-text-secondary uppercase mb-4 flex items-center gap-2">
            <BarChart3 size={16} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             <div className="flex items-center gap-3">
               ...
               <div>
                 <p className="text-xs text-wa-text-secondary">Busiest Time</p>
                 <p className="text-sm font-bold text-gray-800 dark:text-wa-text-primary">
             ...
             <div className="flex items-center gap-3">
               ...
               <div>
                 <p className="text-xs text-wa-text-secondary">Top Emoji</p>
                 <p className="text-sm font-bold text-gray-800 dark:text-wa-text-primary">
          ...
          <div className="bg-wa-panel-bg p-6 rounded-xl shadow-sm border border-wa-divider">
            ...
            <div className="space-y-4">
              {Object.entries(stats.perUser)
                ...
                .map(([user, count]) => (
                  <div key={user}>
                    <div className="flex justify-between text-sm mb-1 text-gray-700 dark:text-wa-text-primary">
            ...
            <div className="space-y-3">
              {stats.byDay.map((count, dayIndex) => (
                <div key={dayIndex} className="flex items-center gap-3">
                  <span className="text-xs text-wa-text-secondary w-8">{days[dayIndex]}</span>
                  ...
                  <span className="text-xs font-medium text-gray-600 dark:text-wa-text-primary w-10 text-right">{count}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {stats.topWords.length > 0 ? (
                stats.topWords.map(([word, count]) => (
                  <div 
                    key={word} 
                    className="px-3 py-1.5 bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center gap-2 border border-wa-divider"
                  >
                    <span className="text-sm font-medium text-gray-700 dark:text-wa-text-primary">{word}</span>
                    <span className="text-[10px] bg-green-500 text-white px-1.5 rounded-full">{count}</span>
                  </div>
                ))
              ) : (
                <p className="text-sm text-wa-text-secondary italic">No word data available</p>
              )}
            </div>
          </div>

          <div className="bg-wa-panel-bg p-6 rounded-xl shadow-sm border border-wa-divider mb-6">
            <h3 className="text-sm font-semibold text-wa-text-secondary uppercase mb-4 flex items-center gap-2">
            ...
            <div className="flex flex-wrap gap-3">
              {stats.topEmojis.length > 0 ? (
                stats.topEmojis.map(([emoji, count]) => (
                  <div 
                    key={emoji} 
                    className="flex flex-col items-center p-2 bg-gray-50 dark:bg-wa-active-chat-bg rounded-lg border border-wa-divider min-w-[50px]"
                  >
                    <span className="text-2xl mb-1">{emoji}</span>
                    <span className="text-xs font-bold text-wa-text-secondary">{count}</span>
                  </div>
                ))
              ) : (
                <p className="text-sm text-wa-text-secondary italic">No emoji data available</p>
              )}
            </div>
          </div>
        </div>
